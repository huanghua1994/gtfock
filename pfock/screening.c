...
int schwartz_screening(PFock_t pfock, BasisSet_t basis)
{
	...
	Buzz_createBuzzMatrix(
        &pfock->bm_screening, MPI_COMM_WORLD, MPI_DOUBLE, 8,
        myrank, nshells, nshells, nprow, npcol,
        pfock->rowptr_sh, pfock->colptr_sh, 1, 0
    );
	...
	Buzz_startBatchUpdate(pfock->bm_screening);
	...
	Buzz_execBatchUpdate(pfock->bm_screening);
	Buzz_stopBatchUpdate(pfock->bm_screening);
	...
	Buzz_startBuzzMatrixReadOnlyEpoch(pfock->bm_screening);
    Buzz_startBatchGet(pfock->bm_screening);
    Buzz_addGetBlockRequest(pfock->bm_screening, 0, nshells, 0, nshells, sq_values, nshells);
    Buzz_execBatchGet(pfock->bm_screening);
    Buzz_stopBatchGet(pfock->bm_screening);
    Buzz_stopBuzzMatrixReadOnlyEpoch(pfock->bm_screening);
	...
	Buzz_destroyBuzzMatrix(pfock->bm_screening);
