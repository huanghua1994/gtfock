...
PFockStatus_t PFock_computeFock(BasisSet_t basis, PFock_t pfock)
{
    ...
    Buzz_accumulateBlockToProcess(pfock->bm_F2, myrank, myrank, 1, 0, sizeX2, F2, sizeX2, 0);
    Buzz_accumulateBlockToProcess(pfock->bm_F3, myrank, myrank, 1, 0, sizeX3, F3, sizeX3, 0);
    ...
    for (int idx = 0; idx < pfock->nprocs - 1; idx++) 
    {
        ...
        if (1 == stealed) 
        {
            ...
            if (vrow != myrow) 
            {
                Buzz_accumulateBlockToProcess(pfock->bm_F1, vpid, vpid, 1, 0, sizeX1, F1, sizeX1, 0);
            } else {
                Buzz_accumulateBlockToProcess(pfock->bm_F1, myrank, myrank, 1, 0, sizeX1, F1, sizeX1, 0);
            }
            ...
            if (vcol != mycol) 
            {
                Buzz_accumulateBlockToProcess(pfock->bm_F2, vpid, vpid, 1, 0, sizeX2, F2, sizeX2, 0);
            } else {
                Buzz_accumulateBlockToProcess(pfock->bm_F2, myrank, myrank, 1, 0, sizeX2, F2, sizeX2, 0);
            }

            Buzz_accumulateBlockToProcess(pfock->bm_F3, vpid, vpid, 1, 0, sizeX3, F3, sizeX3, 0);
...
{    
	...
	Buzz_startBuzzMatrixReadOnlyEpoch(bm_tmp1);
	...
	Buzz_stopBuzzMatrixReadOnlyEpoch(bm_tmp1);

	Buzz_startBuzzMatrixReadOnlyEpoch(bm_tmp2);
	...
	Buzz_stopBuzzMatrixReadOnlyEpoch(bm_tmp2);
